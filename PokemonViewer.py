...
#creating labels in the frames
    lbl_pokemon=ttk.Label(frm_user_input, text ="Pokemon Name/Number:")
    lbl_pokemon.grid(column=0, row=0,padx=3,pady=10)

    lbl_error = ttk.Label(frm_user_input, text='')
    lbl_error.grid(column=0, row=10,columnspan=3, sticky=NW)
    lbl_error.configure(foreground='red')

#Creating place to enter text 
    ent_pokemon = ttk.Entry(frm_user_input)
    ent_pokemon.grid(column=1, row=0,pady=10)

#Creating a function to fetch the pokemon info and assign the correct values to each label
...
#if the text entered doesnt give back a result from the api it will post an error message       
        else:
            lbl_error['text'] = "No pokemon found! Please try again!"
            lbl_height_result['text'] = "N/A"
            lbl_weight_result['text'] = "N/A"
            lbl_name_result['text'] = "N/A"
            lbl_number_result['text'] = "N/A"
            lbl_type_result['text'] = "N/A"
...
#Widegets in the info column
    ...
    lbl_height.grid(column= 100, row=100, pady=10)
    ...
    lbl_weight.grid(column= 100, row=200, pady=10)
    ...
    lbl_type.grid(column= 100, row=300, pady=10)
    lbl_type_result= ttk.Label(frm_info, text="TBD")
    lbl_type_result.grid(column=200,row=300)

    lbl_name = ttk.Label(frm_info, text="Name:")
    lbl_name.grid(column=100,row=50, pady=10)
    lbl_name_result = ttk.Label(frm_info, text='TBD')
    lbl_name_result.grid(column=200, row=50, pady=10)

    lbl_number = ttk.Label(frm_info, text="Number:")
    lbl_number.grid(column=100,row=80, pady=10)
    lbl_number_result = ttk.Label(frm_info, text='TBD')
    lbl_number_result.grid(column=200, row=80,pady= 10)

#Widgets in the stats column
    lbl_Hp= ttk.Label(frm_stats, text="HP")
    lbl_Hp.grid(column= 100,row=100,sticky=E)
    ...
    lbl_Defense.grid(column= 100,row=200,sticky=E)
    ...
    lbl_Attack.grid(column= 100,row=300,sticky=E)
    ...
    lbl_Special_Attack.grid(column= 100,row=400, sticky=E)
    ...
    lbl_Special_Defense.grid(column= 100,row=500, sticky=E)
    ...
    lbl_Speed.grid(column= 100,row=600, sticky=E)
